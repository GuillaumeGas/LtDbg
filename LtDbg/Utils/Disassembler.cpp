...
std::string Disassembler::Disassemble(const char * binaryName, unsigned int startingAddr, unsigned int nbInstructions)
{
	std::stringstream ss;
	unsigned int i = 0;
	...
	while (ud_disassemble(&_udObj) && i++ < nbInstructions)
	{
		...
		if (instId != UD_Iinvalid)
		{
			...
			if (instId == UD_Icall)
			{
				...
				{
                    ss << " (" << ManglingHelper::Demangle(SymbolsHelper::Instance()->GetSymbolFromAddr(binaryName, symAddr).name) << ")";
				}
			}
